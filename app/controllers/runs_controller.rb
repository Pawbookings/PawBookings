class RunsController < ApplicationController
  ...
  def create
    ...
    if run.valid? && kennel.runs.create(kennel_id: kennel.id, size_width: run.size_width, size_length: run.size_length, title: run.title, description: run.description, indoor_or_outdoor: run.indoor_or_outdoor, pets_per_run: run.pets_per_run, price: run.price.to_f, weight_limit: run.weight_limit, number_of_rooms: run.number_of_rooms, type_of_pets_allowed: run.type_of_pets_allowed )
      ...
      error_message = "Unable to create your Run, validation failed."
  ...
  def update
    ...
    if run.valid? && run.save!
      ...
      error_message = "Unable to update your Run, validation failed."
      run.errors.full_messages.each do |err|
        error_message << " #{err}."
      end
      flash[:notice] = error_message
  ...
  def run_params
    return params.require(:run).permit(:size_width, :size_length, :title, :description, :indoor_or_outdoor, :pets_per_run, :price, :weight_limit, :number_of_rooms, :type_of_pets_allowed)
  end
end
