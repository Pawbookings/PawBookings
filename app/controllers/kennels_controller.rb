class KennelsController < ApplicationController
  ...
  def update
    ...
    kennel.avatar = params[:logo] if !params[:logo].nil?
    if kennel.valid? && kennel.save!
      flash[:notice] = "Your Kennel updated successfully!"
      redirect_to kennels_path(id: kennel.id)
    else
  ...
  def filter_reservation_search
    ...
    return @reservation_search_results = Reservation.where(search_by.to_sym => params[:reservation_search].downcase, kennel_id: @kennel[:id]).order(created_at: :desc).limit(10)
  end
